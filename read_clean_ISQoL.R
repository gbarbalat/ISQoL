...
#Load scores
First_Network_REHABASE_tot = read.csv(file="C:/Users/Guillaume/Desktop/ISQoL/data_Lisa_16122022.csv",
                                      header=TRUE, sep=",",#\t
                                      comment.char="#",encoding = "UTF-8")


mat_A_W_Y <- First_Network_REHABASE_tot %>%
  #filter(Dx=="2-Spectre de la SCHIZOPHRENIE")
  dplyr::select(Study.Subject.ID,CENTRE,LISTE_DIAG1R,
  ...
  mutate(across(c(Age,SQoL18_SEL:ISMI_Intern),~ as.numeric(.))) %>%
...
mat=dplyr::select(mat_A_W,-c(Study.Subject.ID,CENTRE,Dx))
mat_dum_NA <- dummy_cols(
  .data=  dplyr::select(mat,where(is.factor)),
  select_columns = NULL,
...
df_all=cbind(dplyr::select(mat_A_W,c(Study.Subject.ID,CENTRE,Dx)),
             dplyr::select(mat_A_W,where(is.numeric)),
             mat_dum_NA
             )
df_all=rename_with(df_all, ~ gsub(" ", "_", .x, fixed = TRUE))
df_all=rename_with(df_all, ~ gsub(">", "", .x, fixed = TRUE))
df_all=rename_with(df_all, ~ gsub("=", "", .x, fixed = TRUE))
df_all=rename_with(df_all, ~ gsub("+", "", .x, fixed = TRUE))
